...
export async function buildHotkey(options: HotkeyBuildOptions): Promise<boolean> {
  const HOTKEY_DIR = join(options.rootDir, 'hotkey');
  ...
  log.info(`Hotkey Version: ${versionInfo.version} (${buildDate})`);

  // Check venv exists
  if (!existsSync(venvPath)) {
    log.error(`Hotkey venv not found at ${venvPath}. Run setup first.`);
    return false;
  }

  // Run PyInstaller if release build
  if (options.release) {
    const spec = join(HOTKEY_DIR, 'Oracle-Hotkey.spec');
    ...
    if (!existsSync(pyinstallerExe)) {
      log.error('PyInstaller not installed in hotkey venv. Run setup first.');
      return false;
    }

    const distPath = join(DEPLOY_DIR, 'build', 'dist');
    const buildPath = join(DEPLOY_DIR, 'build', 'Oracle-Hotkey');

    const buildSuccess = await execCommand(
      [pyinstallerExe, '--distpath', distPath, '--workpath', buildPath, spec],
      HOTKEY_DIR,
      'Building Oracle-Hotkey.exe with PyInstaller',
      options.verbose
    );
    ...
    log.success('Oracle-Hotkey.exe built successfully');
  }

  log.success('Hotkey build completed');
  return true;
}
